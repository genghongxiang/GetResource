...
extern Host_Path *Host_Path_head;
extern Host_Path *Host_Path_tail;
...
int get_video(char *host,char *uri);
...
int get_video(char *host,char *uri)
{
	...
	for(i = 0;i < length;i++)
	{
		...
		if (p != NULL)
		{
			Host_Path *tmpHostPath = NULL;
			tmpHostPath = Host_Path_head;
			while(tmpHostPath)
			{
				if(tmpHostPath->action == 3)
				{
					int j = 0,bfind = 0;
					char *pstr2 = NULL;
					char *pstr = NULL;	
					Item_t * tmpItem_t = NULL;
					tmpItem_t = tmpHostPath->pItem_head;
					while(tmpItem_t != NULL)
					{
						j++;
						pstr = NULL;
						pstr = strstr(uri,tmpItem_t->value);
						if(pstr != NULL)
						{
							bfind = 1;
							if(j == 2)
							{
								pstr2 = strstr(uri,tmpItem_t->value);
							}
						}
						else
						{
				...
				else if(tmpHostPath->action == 1 ||tmpHostPath->action == 2)
				{
					int  bfindflags = 0;
					char *pstr = NULL;	
					Item_t * tmpItem_t = NULL;
					...
					{
						pstr = NULL;
						if(strcmp(tmpItem_t->type,"host") == 0)
						{
							pstr = strstr(host,tmpItem_t->value);
							if(pstr != NULL)
							{
								bfindflags = 1;
								tmpItem_t = tmpItem_t->next;
						...
						else if(strcmp(tmpItem_t->type,"path") == 0)
						{
							pstr = strstr(uri,tmpItem_t->value);
							if(pstr != NULL)
							{
					...
					if(bfindflags == 1)
					{
						if(tmpHostPath->action == 1)
						{
							return 0;
						}
						else if(tmpHostPath->action == 2)
						{
							return -1;
						}
					}
